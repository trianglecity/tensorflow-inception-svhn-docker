...
if __name__ == '__main__':
	...
	for i in xrange(int((len(labels)-batch_size)/(batch_size))):
		...
		probs_val = sess.run( cnn.probs, feed_dict={cnn.imgs: batch_array, cnn.labels: label_array })[0]
		print ".. probs_val = ", probs_val
		
		preds = (np.argsort(probs_val)[::-1])[0:10]
		
    		for p in preds:
        		print class_names[p], probs_val[p]
		print ""
